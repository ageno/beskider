...
- **Tytuły stron:** Wszystkie podstrony zawsze mają `title` w formacie: **Beskider.pl - [nazwa podstrony]**. W front matter (YAML) ustawiać pełny tytuł z prefiksem `Beskider.pl - `. Dotyczy to stron Jekyll (index, 404, partnerzy, regulamin, blog, wpisy z _posts) oraz ewentualnych standalone HTML z własnym `<title>`.
